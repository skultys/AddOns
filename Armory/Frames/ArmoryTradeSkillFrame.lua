--[[
    ...
    Revision: 685 2015-06-24T15:09:16Z
...
function ArmoryTradeSkillFrame_Update()
    ...
    for i = 1, ARMORY_TRADE_SKILLS_DISPLAYED, 1 do
        ...
        if ( skillIndex <= numTradeSkills ) then    
            ...
            if ( (numSkillUps or 0) > 1 and skillType == "optimal" and not displayAsUnavailable ) then
...
function ArmoryTradeSkillFrame_SetSelection(id)
    ...
    local numTopLines = ArmoryTradeSkillRequirementText:GetNumLines();
    if ( ArmoryTradeSkillSkillCooldown:GetText() ) then
        numTopLines = numTopLines + ArmoryTradeSkillSkillCooldown:GetNumLines();
    end
    if ( numTopLines > 2 ) then
        ArmoryTradeSkillDescription:SetPoint("TOPLEFT", 5, -30 - (10 * numTopLines));
    else
        ArmoryTradeSkillDescription:SetPoint("TOPLEFT", 5, -50);
    end
