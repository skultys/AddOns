--[[
    ...
    Revision: 692 2015-07-16T20:53:52Z
...
function ArmoryQuestInfo_ShowRewards()
    ...
    if ( numQuestSpellRewards > 0 ) then
		...
		texture, name, isTradeskillSpell, isSpellLearned, hideSpellLearnText, isBoostSpell, garrFollowerID = Armory:GetQuestLogRewardSpell();
        
        if ( not hideSpellLearnText ) then
            if ( isTradeskillSpell ) then
                learnSpellText:SetText(REWARD_TRADESKILL_SPELL);
            elseif ( isBoostSpell ) then
                learnSpellText:SetText(REWARD_ABILITY);
            elseif ( garrFollowerID ) then
                learnSpellText:SetText(REWARD_FOLLOWER);				
    ...
    if ( numQuestRewards > 0 or numQuestCurrencies > 0 or money > 0 or  xp > 0 or playerTitle ) then
        ...
        -- XP rewards
        lastFrame = ArmoryQuestInfo_ToggleRewardElement(ArmoryQuestInfoXPFrame, BreakUpLargeNumbers(xp), lastFrame);		
        -- Skill Point rewards
        lastFrame = ArmoryQuestInfo_ToggleRewardElement(ArmoryQuestInfoSkillPointFrame, skillPoints, lastFrame);
        ...
        -- Title reward
        lastFrame = ArmoryQuestInfo_ToggleRewardElement(ArmoryQuestInfoPlayerTitleFrame, playerTitle, lastFrame);
        -- Item rewards
        local index;
        local baseIndex = rewardsCount;
