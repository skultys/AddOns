--[[
    ...
    Revision: 700 2015-08-17T15:55:25Z
...
local function EnhanceItemTooltip(tooltip, id, link)
    ...
    elseif ( link ~= fetched ) then
        ...
        local name, link, _, itemLevel, minLevel, itemType, itemSubType, _, equipLoc = GetItemInfo(id or link);
...
local function RegisterTooltipHook(tooltip, idType, hook, reset)
    ...
    elseif ( not tooltipHooks[tooltip] ) then
        ...
        SetTooltipHook(tooltip, "SetInboxItem", _G.GetInboxItemLink);
        SetTooltipHook(tooltip, "SetGuildBankItem", _G.GetGuildBankItemLink);
        SetTooltipHook(tooltip, "SetQuestItem", _G.GetQuestItemLink);

        SetTooltipHook(tooltip, "SetTradeSkillItem", function(index, reagentId)
            if ( reagentId ) then
                return _G.GetTradeSkillReagentItemLink(index, reagentId);
            else
                return _G.GetTradeSkillItemLink(index);
            end
        end);
        ...
        SetTooltipHook(tooltip, "SetGlyphByID", function(id)
            local button = GetMouseFocus();
            if ( button and button.glyphIndex ) then
                return select(6, _G.GetGlyphInfo(button.glyphIndex));
            end
        end);
