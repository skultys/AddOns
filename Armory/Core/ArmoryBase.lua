--[[
    ...
    Revision: 715 2016-01-09T11:08:52Z
...
--]]
        ...
        elseif ( dbVersion == 37 ) then
            dbEntry:SetValue(2, "General", "WeeklyReset", nil);

            upgraded = true;

        -- convert from 38 to 39
        elseif ( dbVersion == 38 ) then
            local oldName = "SummaryCurrency:"..(_G.GetCurrencyInfo(VALOR_CURRENCY));
            local newName = "SummaryCurrency:"..(_G.GetCurrencyInfo(WOD_VALOR_CURRENCY));

            dbEntry:SetValue(2, "General", newName, dbEntry:GetValue("General", oldName));
            dbEntry:SetValue(2, "General", oldName, nil);

            for realm in pairs(ArmoryDB) do
                for character in pairs(ArmoryDB[realm]) do
                    entry = ArmoryDB[realm][character];
                    entry.CurrencyInfo396 = nil;
                end
            end
